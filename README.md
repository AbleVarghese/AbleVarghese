...
### Application architect who ships complete platforms — solo, end-to-end, at production grade.

*I like building complete production systems — and the engineering machines that build them.*

Software engineer since 2014 — banking-grade delivery for CIBC and payments fintechs — now
applying that discipline to a portfolio of
**10+ products built in under two years** — architected, developed, tested, and operated using
**agentic AI engineering systems** I designed myself. Every platform below is real, running
software: full stacks, payment rails, compliance engines, test suites in the thousands.
...
built nights and weekends alongside a full-time payments-fintech role.
...
|---|---|---|
| **[Licentric](https://licentric.com)** | Software licensing & monetization platform — license keys to AI-agent tokens. 3 minutes to first validation vs. hours on incumbents | Ed25519 offline-first signed licenses · declarative Stripe→license mapping (zero webhook code) · Python SDK on PyPI · TypeScript |
| **[LawyerServed](https://lawyerserved.com)** | North America's transparent legal directory — **1.47M+ verified lawyer profiles**, double-blind two-sided marketplace | Next.js 16 · tRPC · Drizzle · Supabase RLS · Meilisearch at 1M+ scale · Stripe · per-jurisdiction compliance rule-packs |
| **[SolveRight](https://solveright.ai)** | Decision-intelligence platform — **155 decision frameworks** run simultaneously, deterministic 0–100 scoring, contradiction detection | Three-phase AI+deterministic hybrid scoring engine · <100ms sensitivity analysis · 7 export formats |
| **[solvemax](https://solvemax.solveright.ai)** | A verdict with receipts — decisions interrogated, scored, red-teamed through **5,000 seeded Monte-Carlo futures**, blind second-pass audit | Next.js · multi-model LLM failover chain · Stripe · seeded deterministic simulation |
| **[ArgusTest](https://argustest.dev)** | iOS device monitoring + AI-assisted debugging for Claude Code / Cursor users — 100% app-agnostic | MCP integration · HMAC-verified webhooks · SHA-256 error dedup (78% noise reduction) · self-healing daemons |
| **[ShellJolt](https://shelljolt.com)** | macOS cockpit for AI coding CLIs — cost, context, usage-limits, model in one live line; stops OOM freezes before they happen | Rust · Ed25519 licensing · adaptive per-session memory engine · cross-CLI (Claude/Codex/Gemini/Grok) |
| **[Keralora](https://keralora.com)** | Invite-only B2B spice-trade platform, Kerala → Canada, with complete buyer↔seller anonymity | Three-wall anonymity architecture (DAL + Postgres RLS + document redaction) · anonymity test suite in CI |
...
- **[ops-dashboard](https://github.com/AbleVarghese/ops-dashboard)** — zero-dependency live SDLC monitor: watches every project's agents, tests, and git state simultaneously over SSE — **open source (MIT)**
- **local-gitlab** — fully self-hosted GitLab CE + runner: $0/month CI/CD any project plugs into
- **Relay** — portable partner-marketing engine: attribution, commissions, payouts — integrates with any product from zero code upward
...
- **Devrule.ai** — enterprise AI-rules middleware: validates code changes against configurable rules before AI tools (Claude Code, Cursor, Copilot) can execute them — multi-tenant RLS, Stripe billing, 40+ test suites
...
**CIBC** (4 yrs) — Senior Application Engineer: architected and shipped five banking applications to production; Java/Python; CI/CD with IBM UrbanCode; Scrum Master; led the Drive Smart iOS/Android app team as PM; built an internal application platform + CMS used across Solutions, Development, QA, Build and Production teams
**StrongBase Capital** — Securities & options trader: quantitative strategies, Greeks-based risk management
